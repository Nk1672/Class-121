...
while(cap.isOpened()):
    ...
    reset1 = cv2.bitwise_and(img, img, mask = mask2)
    reset2 = cv2.bitwise_and(bg, bg, mask = mask1)
    final_output = cv2.addWeighted(reset1, 1, reset2, 1, 0)
